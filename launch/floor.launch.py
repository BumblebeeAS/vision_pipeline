...
def generate_launch_description():
    ...
    launch_objects = [PushRosNamespace("/auv4/floor")]

    pipeline_nodes = [
        Node(
            package="yolo_ros_trt",
            executable="yolo_node",
            name="floor_yolo_node",
            parameters=[config],
        ),
        Node(
            package="pose_estimator",
            executable="floor_pose_estimator_node",
            name="floor_pose_estimator_node",
            parameters=[config],
        ),
        Node(
            ...
            output="screen",
            parameters=[config, {"node_names": ["floor_yolo_node"]}],
        ),
    ]
