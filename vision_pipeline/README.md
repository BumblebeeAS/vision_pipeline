...
Calling the _namespaced_ `manage_nodes` service of type `lifecycle_msgs/srv/ChangeState` ([link](https://docs.ros2.org/humble/api/lifecycle_msgs/srv/ChangeState.html)) applies the specified transition to all the nodes in `node_names` **in order**. If any node fails to transition, the manager returns a failure and does **not** transition subsequent nodes.
